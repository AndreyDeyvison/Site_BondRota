...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Overlay — fecha o menu ao tocar fora dele (apenas mobile/tablet) */}
      {open && (
        <div
      ...
      <aside
        className={`fixed top-0 left-0 h-screen w-64 flex flex-col z-30 select-none transition-transform duration-300 ease-in-out lg:translate-x-0 ${
          open ? 'translate-x-0' : '-translate-x-full'
        }`}
        style={{ background: 'linear-gradient(180deg, #1A2540 0%, #1E2B47 60%, #243355 100%)' }}
      >
        {/* Logo */}
        <div className="flex items-center justify-between gap-3 px-5 py-5 border-b" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
